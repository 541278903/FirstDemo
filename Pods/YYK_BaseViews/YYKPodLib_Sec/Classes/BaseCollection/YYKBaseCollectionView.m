...
#import "YYKBaseCollectionView.h"
#import "UIView+YYK.h"
...
@interface YYKBaseCollectionView()<UICollectionViewDelegate,UICollectionViewDataSource>
@end


@implementation YYKBaseCollectionView
...
-(void)setup{
    ...
    collectionView.contentInset = UIEdgeInsetsMake(15, 30, 0, 30);//分别代表着上边界，左边界，底边界，右边界，扩展出去的值。
//    [collectionView registerNib:[self.cell defaultNib] forCellWithReuseIdentifier:[self.cell defaultIdentifier]];
    if(!self.cell)
    {
        [collectionView registerNib:YYKBaseCollectionViewCell.defaultNib forCellWithReuseIdentifier:self.defaultIdentifier];
    }else
    {
...
- (YYKBaseCollectionViewCell *)cell
{
    if(!_cellName)
    {
