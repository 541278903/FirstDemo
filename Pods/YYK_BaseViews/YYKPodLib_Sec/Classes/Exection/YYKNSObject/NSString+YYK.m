...
#import "NSString+YYK.h"

#import <objc/runtime.h>
#import <CommonCrypto/CommonDigest.h>
#import "NSDate+YYK.h"

@implementation NSString (YYK)

+ (NSString *)yk_translation:(NSString *)arabStr
{
...
- (NSString *)yk_timeInterval
{
    ...
    if (distanceSecond >= 86400) {
        return [self yk_dateTimeWithLonglongTimeFormat:@"yyyy-MM-dd HH:mm:ss"];
...
- (NSString *)yk_dayTimeInterval
{
    ...
    // 返回与现在的时间差
    if (distanceSecond >= 86400) {
        return [self yk_dateTimeWithLonglongTimeFormat:@"yyyy-MM-dd"];
    } else if (distanceSecond >= 3600) {
        return [NSString stringWithFormat:@"%ld小时前",distanceSecond / 3600];
    } else if (distanceSecond >= 60) {
        return [NSString stringWithFormat:@"%ld分钟前",distanceSecond / 60];
...
- (NSString *)yk_timeRemain
{
    ...
    if (remainHour > 0 || remainStr.length != 0){
        [remainStr appendString:[NSString stringWithFormat:@"%@时",@(remainHour)]];
    }
...
- (NSString *)yk_timeRemainMillisecond
{
    ...
    long remainTimeSecond = remainTimeLong / 10;
    long remainMillisecond = remainTimeLong % 10;
    NSInteger remainDay = remainTimeSecond / dayLong;
    NSInteger remainHour = (remainTimeSecond % dayLong) / hourLong;
    NSInteger remainMinute = (remainTimeSecond % hourLong) / minuteLong;
    NSInteger remainSecond = (remainTimeSecond % minuteLong);
    
    NSMutableString *remainStr = @"".mutableCopy;
    if (remainDay > 0){
        [remainStr appendString:[NSString stringWithFormat:@"%@:",@(remainDay)]];
    }
    
    if (remainHour > 0 || remainStr.length != 0){
        [remainStr appendString:[NSString stringWithFormat:@"%@:",@(remainHour)]];
    }
    
    if (remainMinute > 0 || remainStr.length != 0){
        [remainStr appendString:[NSString stringWithFormat:@"%@:",@(remainMinute)]];
    }
    
    if (@(remainSecond).stringValue.length == 1) {
        [remainStr appendString:@"0"];
    }
    [remainStr appendString:[NSString stringWithFormat:@"%@:",@(remainSecond)]];
    [remainStr appendString:[NSString stringWithFormat:@"%@",@(remainMillisecond)]];
    return remainStr;
}
...
+ (NSString *)yk_distanceWithTime:(NSString *)longLongTime
{
    ...
    if (distanceSecond > 0) {
        ...
        NSInteger month = date.yk_month;
        NSInteger day = date.yk_day;
        NSInteger hour = date.yk_hour;
        NSInteger minute = date.yk_minute;
        if (date.yk_year == now.yk_year &&
            date.yk_month == now.yk_month) { // 同年同月
           if (day == now.yk_day) {
               return [NSString stringWithFormat:@"今天%zd:%zd",hour,minute];
           } else if (day == now.yk_day + 1) {
               return [NSString stringWithFormat:@"明天%zd:%zd",hour,minute];
           } else if (day == now.yk_day + 2) {
               return [NSString stringWithFormat:@"后天%zd:%zd",hour,minute];
           } else {
               return [NSString stringWithFormat:@"本月%zd日 %zd:%zd",day,hour,minute];
           }
        } else if(date.yk_year == now.yk_year &&
                  date.yk_month != now.yk_month){ // 同年不同月
           return [NSString stringWithFormat:@"%zd/%zd %zd:%zd",month,day,hour,minute];
        } else {
           return [NSString stringWithFormat:@"%zd/%zd/%zd %zd:%zd",date.yk_year,month,day,hour,minute];
        }
    }
}
...
- (NSAttributedString *)yk_stringWithLineSpacing:(CGFloat)lineSpacing fontSize:(CGFloat)fontSize color:(UIColor *)color
{
    ...
    NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:self attributes:@{NSParagraphStyleAttributeName:style,
...
- (instancetype)yk_complementHexString
{
    ...
    // 再转回十六进制
    NSString *result = [self yk_hexStringWithDenary:hexValue formatLength:4];
...
{
    return [self yk_isPureInt] || [self yk_isPureFloat];
}
...
{
    NSMutableAttributedString *attrString = [[NSMutableAttributedString alloc] initWithString:totalString attributes:attribute];
    [highlightAttribute enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        [attrString addAttribute:key value:obj range:[totalString rangeOfString:hightlightString]];
    }];
    return attrString;
}
...
{
    NSMutableAttributedString *attrString = [[NSMutableAttributedString alloc] initWithString:self attributes:attribute];
    [highlightAttribute enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        [attrString addAttribute:key value:obj range:[self rangeOfString:hightlightString]];
    }];
    return attrString;
}
...
{
    NSMutableAttributedString *attrString = [[NSMutableAttributedString alloc] initWithString:self attributes:attribute];
    [hightlightArray enumerateObjectsUsingBlock:^(NSString *  _Nonnull hightlightString, NSUInteger idx, BOOL * _Nonnull stop) {
        [highlightAttribute enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
            [attrString addAttribute:key value:obj range:[self rangeOfString:hightlightString]];
        }];
    }];
