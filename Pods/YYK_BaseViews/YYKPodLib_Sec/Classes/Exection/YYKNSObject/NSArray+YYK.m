...
+ (void)load
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        [obj yk_swizzleMethod:@selector(objectAtIndex:) withMethod:@selector(safeObjectAtIndex:)];
    });
}
...
- (NSArray *)yk_fetchPropertys:(NSString *)propertyName defaultValue:(id)defaultValue {
    NSMutableArray *propertyLists = [NSMutableArray array];
    for (NSObject *object in self) {
        unsigned int count = 0;
        Class c = object.class;
